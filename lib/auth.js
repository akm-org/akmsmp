...
const { Users, PERMANENT_ADMIN_EMAILS } = require('./db');
...
function isAdminUser(u) {
  ...
  return PERMANENT_ADMIN_EMAILS.includes(String(u.email || '').toLowerCase());
}

function requireAdmin(req, res, next) {
  const u = getUser(req);
  if (!u) return res.status(401).json({ error: 'Not signed in' });
  if (!isAdminUser(u)) return res.status(403).json({ error: 'Admin only' });
  req.user = u;
  next();
}
...
function adminEmails() {
  ...
  return Array.from(new Set([...PERMANENT_ADMIN_EMAILS, ...fromEnv]));
}

module.exports = { setSession, clearSession, getUser, requireAuth, requireAdmin, isAdminUser, hash, compare, adminEmails };
